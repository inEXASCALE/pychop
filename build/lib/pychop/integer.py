...
class FakeQuantizeSTE(Function):
    ...
    @staticmethod
    def forward(ctx, input: torch.Tensor, chop: Chopi):
        # Forward: 真正的 integer quant + dequant (模拟硬件)
        q = chop.quantize(input)
        dq = chop.dequantize(q)
        # 保存 chop 以便 backward（虽然 STE 不需要）
        ctx.chop = chop
        return dq
    ...
    def backward(ctx, grad_output: torch.Tensor):
        # STE 核心：梯度直接通过（模拟 "直通"）
        return grad_output, None   # None 对应 chop 参数


class ChopiSTE(Chopi):
    """
    Chopi with built-in Straight-Through Estimator.
    推荐在训练时使用此版本（完全兼容你原来的 Chopi(bits=..., symmetric=...) 调用）。
    """
    def __init__(self, bits: int = 8, symmetric: bool = False, **kwargs):
        super().__init__(bits=bits, symmetric=symmetric, **kwargs)

    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        """Fake quantization with STE (推荐用法：chop(x) 即可)"""
        if x is None or x.numel() == 0:
            return x
        return FakeQuantizeSTE.apply(x, self)

    # 保留原始方法（inference / 调试时可用）
    def quantize(self, x):
        return super().quantize(x)
    
    def dequantize(self, x):
        return super().dequantize(x)