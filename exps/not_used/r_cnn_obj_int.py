...
def get_quantized_faster_rcnn(quantizer):
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            ...
            param.copy_(quantizer.dequantize(quantized_param))
    
    return model.cuda()


def get_quantized_faster_rcnn_chop(chop):
    """Load and quantize Faster R-CNN model weights."""
    model = fasterrcnn_resnet50_fpn(weights=FasterRCNN_ResNet50_FPN_Weights.DEFAULT)
    model.eval()
        ...
        for i, (images, targets) in enumerate(loader):
            ...
            try:
                ...
                for output in outputs:
                    ...
                    #    output["boxes"] = quantizer.dequantize(quantizer.quantize(output["boxes"]))
                    #    output["scores"] = quantizer.dequantize(quantizer.quantize(output["scores"]))
...
if __name__ == "__main__":
    ...
    quantizer_int8 = Chopi(num_bits=8, symmetric=False)

    quantizer_int16 = Chopi(num_bits=16, symmetric=False)

    # model_quant_int4 = get_quantized_faster_rcnn(quantizer_int4)
    
    model_quant_int8 = get_quantized_faster_rcnn(quantizer_int8)

    model_quant_int16 = get_quantized_faster_rcnn(quantizer_int16)
    ...
    print(f"Running quantized inference (simulated {quantizer_int8.num_bits}-bit)...")
    latencies_quant8, preds_quant8, img_quant8, out_quant8, ids_quant8 = run_inference(model_quant_int8, 
                                    loader, use_amp=False, max_images=max_images, quantizer=quantizer_int8)
    
    map_quant8 = evaluate_coco(preds_quant8, coco_gt, ids_quant8)


    print(f"Running quantized inference (simulated {quantizer_int16.num_bits}-bit)...")
    latencies_quant16, preds_quant16, img_quant16, out_quant16, ids_quant16 = run_inference(model_quant_int16, 
                                    loader, use_amp=False, max_images=max_images, quantizer=quantizer_int16)
    
    map_quant16 = evaluate_coco(preds_quant16, coco_gt, ids_quant16)

    # Print Results
    print(f"FP32 - Latency: {np.mean(latencies_fp32):.2f} ± {np.std(latencies_fp32):.2f} ms, mAP: {map_fp32:.3f}")
    # print(f"Quantized ({quantizer_int4.num_bits}-bit) - Latency: {np.mean(latencies_quant4):.2f} ± {np.std(latencies_quant4):.2f} ms, mAP: {map_quant4:.3f}")
    print(f"Quantized ({quantizer_int8.num_bits}-bit) - Latency: {np.mean(latencies_quant8):.2f} ± {np.std(latencies_quant8):.2f} ms, mAP: {map_quant8:.3f}")
    print(f"Quantized ({quantizer_int16.num_bits}-bit) - Latency: {np.mean(latencies_quant16):.2f} ± {np.std(latencies_quant16):.2f} ms, mAP: {map_quant16:.3f}")

    # save_results_to_csv(latencies_fp32, latencies_quant, map_fp32, map_quant, quantizer_int8)
    filename = "obj_detect_results.csv"
    ...
    with open(filename, "w", newline="") as f:
        ...
        writer.writerow([f"Quantized ({quantizer_int8.num_bits}-bit)", f"{np.mean(latencies_quant8):.2f}", f"{np.std(latencies_quant8):.2f}", f"{map_quant8:.3f}"])
        writer.writerow([f"Quantized ({quantizer_int16.num_bits}-bit)", f"{np.mean(latencies_quant16):.2f}", f"{np.std(latencies_quant16):.2f}", f"{map_quant16:.3f}"])
    ...
    batch_size = len(img_quant16)
    targets = [loader.dataset[i][1] for i in range(batch_size)]
    # plot_detection_all(img_quant4, out_quant4, targets, prefix=f"detection_quant_{quantizer_int4.num_bits}bit")
    plot_detection_all(img_quant8, out_quant8, targets, prefix=f"detection_quant_{quantizer_int8.num_bits}bit")
    plot_detection_all(img_quant16, out_quant16, targets, prefix=f"detection_quant_{quantizer_int16.num_bits}bit")
