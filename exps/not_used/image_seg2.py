...
def visualize_results(images, masks, predictions, num_images=3, save_dir='results'):
    ...
    fig, axes = plt.subplots(1, num_images, figsize=(6 * num_images, 10))  # Horizontal layout: 1 row, num_images columns
    
    # Ensure axes is iterable even for num_images=1
    if num_images == 1:
        axes = [axes]
    ...
    for i in range(num_images):
        ...
        print(f"Image {i} - Mask shape: {mask.shape}, unique values: {np.unique(mask)}")
        print(f"Image {i} - Pred shape: {pred.shape}, unique values: {np.unique(pred.round(2))}")
        
        # Display original image
        axes[i].imshow(img)
        
        # Ground-truth mask (green, transparent)
        gt_colored = np.zeros((mask.shape[0], mask.shape[1], 3), dtype=np.float32)
        gt_mask = (mask > 0.5)  # Binary mask
        gt_colored[gt_mask] = [0, 1, 0]  # Green (normalized to 0-1 range)
        axes[i].imshow(gt_colored, alpha=0.4)  # Overlay with transparency
        
        # Predicted mask (red, transparent)
        pred_colored = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.float32)
        pred_mask = (pred > 0.5)  # Binary mask
        pred_colored[pred_mask] = [1, 0, 0]  # Red (normalized to 0-1 range)
        axes[i].imshow(pred_colored, alpha=0.4)  # Overlay with transparency
        
        # Title and axis
        axes[i].set_title(f'Image {i+1}: Original + Ground Truth (Green) + Prediction (Red)', fontsize=12)
        axes[i].axis('off')
        
        # Save the overlaid image
        plt.imsave(f'{save_dir}/image_{i}_overlay.png', img)  # Save original image first
        # Capture the rendered figure with overlays
        fig.canvas.draw()
        overlay_img = np.frombuffer(fig.canvas.buffer_rgba(), dtype=np.uint8)
        overlay_img = overlay_img.reshape(fig.canvas.get_width_height()[::-1] + (4,))  # RGBA
        overlay_img = overlay_img[:, :, :3]  # Convert to RGB by dropping alpha
        plt.imsave(f'{save_dir}/image_{i}_overlay_with_masks.png', overlay_img)  # Save with overlays
