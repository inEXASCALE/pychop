...
def replace_for_qat(model: nn.Module, chop: LightChop, quant_act: bool = False):
    ...
    for name, module in list(model.named_children()):
        ...
        if isinstance(module, nn.Conv2d):
            # Do not quantize activation for the very first conv layer
            q_act = quant_act and ('conv1' not in name)
            q_module = QATConv2d(module, chop, quant_act=q_act)
            setattr(model, name, q_module)
            replaced = True
        elif isinstance(module, nn.Linear):
            # Do not quantize activation before the final classifier
            q_act = quant_act and ('fc' not in name)
            q_module = QATLinear(module, chop, quant_act=q_act)
            setattr(model, name, q_module)
            replaced = True
        
        if not replaced:
            # Recurse into sub-modules (e.g., BasicBlock, Sequential)
