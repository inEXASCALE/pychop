...
class FPRound_:
    def __init__(self, ibits=4, fbits=4, rmode=1):
        ...
        self.fbits = fbits
        self.rmode = rmode
        self._np_impl = NPFPRound(ibits=ibits, fbits=fbits, rmode=rmode)
    ...
    def get_format_info(self):
        return self._np_impl.get_format_info()
...
for _name in _UNARY_METHODS:
    setattr(FPRound_, _name, _make_unary(_name))

for _name in _BINARY_METHODS:
    setattr(FPRound_, _name, _make_binary(_name))

for _name in _REDUCTION_METHODS:
    setattr(FPRound_, _name, _make_reduction(_name))
