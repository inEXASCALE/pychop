...
class quant():
    ...
    def __init__(self, bits=8, sign=1, zpoint=1, rd_func=None, clip_range=None, epsilon=1e-12):
        ...
        if self.rd_func is None:
            self.rd_func = lambda x: torch.round(x, decimals=0)
            
        if self.clip_range is None:
            self.clip_func = lambda x: torch.clamp(x, min=self.alpha_q, max=self.beta_q)
        else:
            self.clip_func = lambda x: torch.clamp(x, min=self.clip_range[0], max=self.beta_q[self.clip_range[1]])
    ...
    def quantization(self, x, s, z):
        ...
        x_q = self.clip_func(x_q).type(self.intType)
        return x_q

    def dequantization(self, x_q, s, z):
        x_q = x_q.type(torch.float32)
        x = s * x_q + z
        return x
