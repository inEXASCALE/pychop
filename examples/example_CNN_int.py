...
# IQuantized CNN
class IQuantizedCNN(nn.Module):
    def __init__(self, chop):
        super().__init__()
        self.conv1 = IQuantizedConv2d(1, 16, 3, chop=chop)
        self.pool = IQuantizedMaxPool2d(2, chop=chop)
        self.conv2 = IQuantizedConv2d(16, 32, 3, chop=chop)
        # self.pool2 = IQuantizedMaxPool2d(2, chop)
        self.fc1 = IQuantizedLinear(32 * 5 * 5, 128, chop=chop)
        # self.dropout = IQuantizedDropout(0.5, chop)
...
def test_layers():
    ...
    qconv1d = IQuantizedConv1d(3, 6, 3, chop=chop).to(device)
    ...
    qconv3d = IQuantizedConv3d(3, 6, 3, chop=chop).to(device)
    ...
    qmaxpool1d = IQuantizedMaxPool1d(2, chop=chop).to(device)
    x = torch.randn(1, 3, 32, device=device)
    ...
    qmaxpool2d = IQuantizedMaxPool2d(2, chop=chop).to(device)
    ...
    qmaxpool3d = IQuantizedMaxPool3d(2, chop=chop).to(device)
    x = torch.randn(1, 3, 16, 16, 16, device=device)
    ...
    qavgpool = IQuantizedAvgPool(2, chop=chop).to(device)
    x = torch.randn(1, 3, 32, 32, device=device)
    ...
    qavgpool1d = IQuantizedAvgPool1d(2, chop=chop).to(device)
    x = torch.randn(1, 3, 32, device=device)
    ...
    qavgpool2d = IQuantizedAvgPool2d(2, chop=chop).to(device)
    x = torch.randn(1, 3, 32, 32, device=device)
    ...
    qlstm = IQuantizedLSTM(10, 20, chop=chop).to(device)
    ...
    qattn = IQuantizedAttention(embed_dim, num_heads, chop=chop).to(device)
    ...
    qdropout = IQuantizedDropout(0.5, chop=chop).to(device)
...
if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    import pychop; pychop.backend("torch")
    # Load MNIST dataset
